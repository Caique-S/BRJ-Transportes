...
export async function GET() {
  try {
    ...
    console.log(
      `📊 Buscando progresso para turno: ${turnoAtual} (hora: ${horaAtual})`,
    );
    ...
    // ✅ CORREÇÃO: Query para carregamentos concluídos COM finalizadoComBotao: true
    const carregamentosConcluidos =
      await carregamentosCollection.countDocuments({
        $and: [
          ...
          },
          {
            $and: [
              { status: "concluido" },
              { "metadata.finalizadoComBotao": true }, // ← ADICIONAR FILTRO
            ],
          },
        ],
      });
    ...
    if (totalPrevisto === 0) {
      ...
      console.log(
        `📈 Total carregamentos no turno: ${totalCarregamentosTurno}`,
      );
    ...
    console.log(
      `📊 Resultado: ${carregamentosConcluidos}/${totalPrevisto} = ${percentualFinal}%`,
    );
